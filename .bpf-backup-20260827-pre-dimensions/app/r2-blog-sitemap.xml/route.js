...
export async function GET() {
  ...
  const entries = mergeSitemapEntries(
    ...
    localHtmlEntries('blog')
  );
  const urls = entries
    .map(({ slug, lastmod }) => `  <url><loc>${escapeXml(`${siteUrl()}/blog/${slug}.html`)}</loc>${lastmod ? `<lastmod>${lastmod}</lastmod>` : ''}<changefreq>weekly</changefreq><priority>0.88</priority></url>`)
    .join('\n');

...
export async function GET() {
  ...
  const translated = new Set(translatedPaths());
  const alternatesFor = slug => {
    const rel = `products/${slug}.html`;
    if (!translated.has(rel)) return '';
    const rows = [['en', localeUrl('en', rel)]];
    for (const code of LOCALES) rows.push([code, localeUrl(code, rel)]);
    rows.push(['x-default', localeUrl('en', rel)]);
    return rows
      .map(([hreflang, href]) => `<xhtml:link rel="alternate" hreflang="${escapeXml(hreflang)}" href="${escapeXml(href)}"/>`)
      .join('');
  };
  const urls = entries
    .map(({ slug, lastmod }) => `  <url><loc>${escapeXml(`${siteUrl()}/products/${slug}.html`)}</loc>${lastmod ? `<lastmod>${lastmod}</lastmod>` : ''}<changefreq>weekly</changefreq><priority>0.90</priority>${alternatesFor(slug)}</url>`)
    .join('\n');

...
import { REDIRECTED_PRODUCT_SLUGS } from './indexing-policy.js';
...
function isRedirectedProduct(url) {
  try {
    const match = new URL(url).pathname.match(/^\/products\/([^/]+)\.html$/i);
    return Boolean(match && REDIRECTED_PRODUCT_SLUGS.has(match[1]));
  } catch {
    return false;
  }
}
...
export function buildImageSitemap() {
  ...
  for (const file of walkHtml(CONTENT_ROOT).sort()) {
    ...
    if (isRedirectedProduct(canonical)) continue;

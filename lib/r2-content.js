...
import { SITE_URL, forceWww, stripHtml } from './seo-utils';
...
export function metadataFromContent(content, type, slug) {
  ...
  const title = stripHtml(content?.seoTitle || content?.title || content?.headline || fallbackTitle);
  const description = stripHtml(content?.metaDescription || content?.description || content?.quickAnswer || 'BestPackFactory packaging content for B2B buyers.');
  const canonical = forceWww(content?.canonical || `${siteUrl()}${contentPath(t, clean)}`);

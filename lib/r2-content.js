...
async function contentLocation(type) {
  return resolveR2TypeLocation(type, {
    prefix: typePrefix(type),
    indexPath: indexPath(type),
...
export async function getR2Content(type, slug) {
  ...
  const location = await contentLocation(t);
  if (!location) return null;
  const tags = [t, contentIndexTag(t), contentTag(t, clean), ...(location.releaseId ? [`r2-release:${location.releaseId}`] : [])];
  const json = await fetchJson(`${baseUrl()}/${location.prefix}/${encodeURIComponent(clean)}.json`, tags);
  if (json) return json;
  const html = await fetchText(`${baseUrl()}/${location.prefix}/${encodeURIComponent(clean)}.html`, tags);
...
export async function getR2ContentIndex(type) {
  ...
  return fetchJson(
    ...
    [t, contentIndexTag(t), 'content:index', ...(location.releaseId ? [`r2-release:${location.releaseId}`] : [])],
  );
}

...
import { getLeadPage, listLeadPageRoutes } from './lead-pages';
...
const WHATSAPP_TEXTS = {
  ar: 'مرحباً BestPackFactory، أحتاج إلى عرض سعر لتغليف مخصص.',
  de: 'Hallo BestPackFactory, ich benötige ein Angebot für individuelle Verpackungen.',
  es: 'Hola BestPackFactory, necesito una cotización de empaques personalizados.',
  fr: "Bonjour BestPackFactory, j'ai besoin d'un devis pour des emballages personnalisés.",
  ja: 'こんにちは BestPackFactory、カスタム包装のお見積もりをお願いしたいです。',
};

function langFromRoute(routePath = '') {
  const first = String(routePath || '').split('/')[0];
  return LANG_PREFIXES.has(first) ? first : null;
}

function applyLangFixes(body, lang) {
  if (!lang) return body;

  // Rewrite internal links that are missing the language prefix
  body = body.replace(
    /(\shref=["'])\/(?!(ar|de|es|fr|ja|assets|css|js|api|_next)\/)([^"']*)/g,
    (_, attrPrefix, __, p) => `${attrPrefix}/${lang}/${p}`
  );

  // Localize WhatsApp pre-fill text
  const waText = WHATSAPP_TEXTS[lang];
  if (waText) {
    body = body.replace(
      /(https?:\/\/wa\.me\/\d+\?text=)[^"'&]*/g,
      (_, waPrefix) => waPrefix + encodeURIComponent(waText)
    );
  }

  // Wrap Arabic pages with RTL direction
  if (lang === 'ar' && !body.includes('dir="rtl"') && !body.includes("dir='rtl'")) {
    body = `<div dir="rtl" lang="ar">${body}</div>`;
  }
...
function headingFromMetadata(metadata = {}, routePath = '') {
  const title = stripHtml(metadata.title || '')
    .replace(/\s*\|\s*BestPackFactory\s*$/i, '')
    .replace(/\s*\|\s*BPF\s*$/i, '')
    .replace(/\s*\|\s*MOQ\s*500(?:\s*PCS)?\s*$/i, '')
    .replace(/\s*\|\s*Custom Packaging\s*$/i, '')
    .trim();
  if (title) return title;
  return cleanRoute(routePath)
    .replace(/\.html$/i, '')
    .replace(/^products\//i, '')
    .replace(/-/g, ' ')
...
function ensureSemanticH1(body = '', metadata = {}, routePath = '') {
  if (/<h1\b/i.test(body)) return body;
  const heading = headingFromMetadata(metadata, routePath);
  if (!heading) return body;
  return `<h1 class="bpf-seo-h1">${escapeHtml(heading)}</h1>\n${body}`;
}
...
function productImageFromRoute(routePath = '') {
  const clean = cleanRoute(routePath);
  if (!clean.startsWith('products/')) return undefined;
  const slug = clean.replace(/^products\//, '').replace(/\.html$/i, '');
  const candidates = [
    `assets/products/${slug}-01.avif`,
    `assets/products/${slug}-01.webp`,
    `assets/products/${slug}-01.jpg`,
    `assets/products/${slug}.avif`,
    `assets/products/${slug}.webp`,
    `assets/products/${slug}.jpg`
  ];
  const match = candidates.find(publicAssetExists);
  return match ? siteUrl(`/${match}`) : undefined;
}

function isUsablePageImage(url = '') {
  const clean = String(url || '').split(/[?#]/)[0];
  if (!clean || /^data:/i.test(clean)) return false;
  return !/(?:^|\/)(?:logo|favicon)[^/]*\.(?:svg|png|jpe?g|webp|avif)$/i.test(clean);
}

function firstImageUrl(html, routePath = '') {
  const routeProduct = productImageFromRoute(routePath);
  if (routeProduct) return routeProduct;
  const og = html.match(/<meta\s+(?:property|name)=["']og:image["']\s+content=["']([^"']+)["']/i) ||
    html.match(/<meta\s+content=["']([^"']+)["']\s+(?:property|name)=["']og:image["']/i);
  if (og?.[1] && isUsablePageImage(og[1])) return absoluteImageUrl(og[1]);
  const images = [...html.matchAll(/<img\b[^>]*\bsrc=["']([^"']+)["']/gi)].map(match => match[1]);
  const productImage = images.find(src => /(?:^|\/)assets\/products\//i.test(src) && isUsablePageImage(src));
  const firstUsable = productImage || images.find(isUsablePageImage);
  return firstUsable ? absoluteImageUrl(firstUsable) : undefined;
}
...
function fileDateForRoute(routePath) {
  try {
    const file = resolveHtmlFile(routePath);
...
function productPropertiesFromHtml(html = '') {
  const properties = [];
  const rowRe = /<tr\b[^>]*>[\s\S]*?<th\b[^>]*>([\s\S]*?)<\/th>[\s\S]*?<td\b[^>]*>([\s\S]*?)<\/td>[\s\S]*?<\/tr>/gi;
  let match;
...
function productJsonLdFromHtml(html, routePath, metadata) {
  ...
  const image = firstImageUrl(html, routePath);
  const additionalProperty = productPropertiesFromHtml(html);
  return JSON.stringify({
    '@context': 'https://schema.org',
    '@type': 'Product',
    name,
    ...
    sku: cleanRoute(routePath).replace(/^products\//, '').replace(/\.html$/i, '').toUpperCase(),
    ...
    offers: {
      ...
      url: canonical,
      seller: { '@type': 'Organization', name: 'BestPackFactory', url: SITE_URL }
    }
  });
}

function productListJsonLdFromHtml(html) {
  const items = [];
  const seen = new Set();
  const re = /<a\b[^>]*href=["']([^"']*\/?products\/[^"']+\.html)["'][^>]*>[\s\S]*?<h3\b[^>]*>([\s\S]*?)<\/h3>/gi;
  let match;
  while ((match = re.exec(html))) {
  ...
  return JSON.stringify({
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: 'BestPackFactory Custom Packaging Products',
...
function articleJsonLdFromHtml(html, routePath, metadata) {
  ...
  const canonical = metadata?.alternates?.canonical || canonicalForRoute(routePath);
  const image = firstImageUrl(html, routePath);
  const dateModified = fileDateForRoute(routePath);
  return JSON.stringify({
    '@context': 'https://schema.org',
    '@type': 'Article',
    headline,
...
function addFallbackJsonLd(html, routePath, metadata, jsonLd) {
  const clean = cleanRoute(routePath);
  const output = [...jsonLd];
  if ((clean === 'index.html' || clean === '') && !hasSchemaType(output, 'Organization')) output.push(organizationJsonLd());
  if ((clean === 'index.html' || clean === '') && !hasSchemaType(output, 'WebSite')) output.push(websiteJsonLd());
  if (clean.startsWith('products/') && !hasSchemaType(output, 'Product')) output.push(productJsonLdFromHtml(html, clean, metadata));
  if (clean === 'products.html' && !hasSchemaType(output, 'ItemList')) {
    const itemList = productListJsonLdFromHtml(html);
    if (itemList) output.push(itemList);
  }
  ...
  return output.map((json) => normalizeJsonLdForRoute(json, clean)).filter(Boolean).map(forceWww);
}

function normalizeJsonLdForRoute(json, routePath) {
  try {
    const parsed = JSON.parse(json);
    if (routePath.startsWith('products/')) normalizeProductJsonLd(parsed);
...
function normalizeProductJsonLd(node) {
  ...
  nodes.forEach((item) => {
    ...
    // Remove price=0 which Google flags as invalid for B2B products
    if (String(offer.price || '') === '0.00' || String(offer.price || '') === '0') {
      delete offer.price;
      delete offer.priceCurrency;
    }
    // Remove PriceSpecification that only has @type and/or description — these are not valid
    // schema.org properties for PriceSpecification and cause Google Rich Results errors.
    // Valid PriceSpecification fields: price, priceCurrency, minPrice, maxPrice, priceType, etc.
    if (offer.priceSpecification && typeof offer.priceSpecification === 'object') {
      const ps = offer.priceSpecification;
      const validPsKeys = ['price', 'priceCurrency', 'minPrice', 'maxPrice', 'eligibleQuantity',
        'eligibleTransactionVolume', 'priceType', 'validFrom', 'validThrough', 'unitCode'];
      const hasValidKey = validPsKeys.some(k => k in ps);
      if (!hasValidKey) {
...
export function pageFromHtml(html, routePath = '') {
  ...
  const lang = langFromRoute(routePath);
  const metadata = metadataFromHtml(html, routePath);
  body = ensureSemanticH1(body, metadata, routePath);
  body = applyLangFixes(body, lang);
...
export function getPage(routePath = '') {
  ...
  if (html) return pageFromHtml(html, routePath);
  return getLeadPage(cleanRoute(routePath));
}
...
export function listHtmlRoutes() {
  ...
  walk(CONTENT_ROOT);
  return [...new Set([...routes, ...listLeadPageRoutes()])].sort();
}

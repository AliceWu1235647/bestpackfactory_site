...
function firstVisibleContentImage(body = '') {
...
export function normalizeProductJsonLd(jsonLd = [], body = '', metadata = {}) {
  const image = metadataImageUrl(metadata) || firstVisibleContentImage(body);
  ...
  return jsonLd.map(raw => {
    ...
    function visit(node) {
      ...
      if (types.includes('Product')) {
        // Custom B2B packaging is quoted only after the specification is known.
        // A price-less or placeholder Offer is not a truthful purchase offer.
        ...
        if (node.manufacturer?.['@type'] === 'Organization' &&
            node.manufacturer.name === 'BestPackFactory') {
          if (!node.manufacturer['@id']) {
            node.manufacturer['@id'] = `${SITE_URL}/#organization`;
            changed = true;
          }
          if (!node.manufacturer.url) {
            node.manufacturer.url = `${SITE_URL}/`;
            changed = true;
          }
          if (!node.manufacturer.logo) {
            node.manufacturer.logo = { '@type': 'ImageObject', url: PUBLISHER_LOGO_URL };
            changed = true;
          }
        }
      }
...
function canonicalWithExistingHash(canonical, value = '') {
  try {
    const hash = new URL(value, SITE_URL).hash;
...
export function normalizePageEntityJsonLd(jsonLd = [], metadata = {}) {
  const canonical = metadata?.alternates?.canonical || '';
  if (!canonical) return jsonLd;
  ...
  return jsonLd.map(raw => {
    ...
    for (const node of nodes) {
      ...
      const rawType = node['@type'];
      const types = Array.isArray(rawType) ? rawType : [rawType];
      const isArticle = types.some(type => ARTICLE_SCHEMA_TYPES.has(type));
      const isProduct = types.includes('Product');
      const isWebPage = types.some(type => ['WebPage', 'AboutPage', 'ContactPage'].includes(type));
      if (!isArticle && !isProduct && !isWebPage) continue;
      ...
      if (node.mainEntityOfPage) {
        if (typeof node.mainEntityOfPage === 'string' && node.mainEntityOfPage !== canonical) {
          node.mainEntityOfPage = canonical;
          changed = true;
        } else if (typeof node.mainEntityOfPage === 'object' &&
                   node.mainEntityOfPage['@id'] !== canonical) {
          node.mainEntityOfPage['@id'] = canonical;
          changed = true;
        }
      }
      if (isWebPage && node['@id']) {
        const normalizedId = canonicalWithExistingHash(canonical, node['@id']);

...
/**
 * Self-locking mailer ("wing lock" shipping box) — no tape or glue. The two
 * long walls rise past the body height into a pair of chamfered lid flaps
 * that meet at the centre; a friction tab on one flap tucks through a slot
 * cut into the other. Short end walls carry locking ears, same technique as
 * trayBox, so the corners stay closed without adhesive either.
 */
export function selfLockingMailerBox(p) {
  const L = Number(p.length);
  const W = Number(p.width);
  const H = Number(p.height);
  const t = Number(p.thickness) || 3;
  ...
  const relief = clamp(t + 0.5, 0.8, 3);
  const ear = clamp(H - a, 6, H);
  const flap = clamp(W / 2 + a + relief * 2, 14, W);
  const chamfer = clamp(flap * 0.3, 6, 20);
  const tabW = clamp(L * 0.22, 18, 40);
  const tabDepth = clamp(flap * 0.45, 8, 18);
  const tabTaper = clamp(tabW * 0.2, 2, 6);
  const slotW = tabW + clamp(t * 1.5, 1, 4);
  const slotDepth = clamp(tabDepth * 0.6, 5, 12);
  const slotInset = clamp(flap * 0.15, 4, 12);

  const xBase = wall;
  const xBaseR = xBase + L;
  const centreX = (xBase + xBaseR) / 2;

  const yFrontFold = flap;
  const yBase = yFrontFold + wall;
  const yBaseT = yBase + W;
  const yBackFold = yBaseT + wall;
  const top = yBackFold + flap;
  ...
  const e = [
    ...
    line(LAYERS.FOLD, xBase, yBase, xBase, yBaseT),
    line(LAYERS.FOLD, xBaseR, yBase, xBaseR, yBaseT),
    line(LAYERS.FOLD, xBase, yFrontFold, xBaseR, yFrontFold),
    line(LAYERS.FOLD, xBase, yBackFold, xBaseR, yBackFold)
  ];

  // Short end walls, open on the outer edge like trayBox's side walls.
  e.push(poly(LAYERS.CUT, [
    [xBase, yBase + relief], [xBase - wall, yBase + relief],
    [xBase - wall, yBaseT - relief], [xBase, yBaseT - relief]
  ]));
  ...
  // Locking ears on the long walls, tucking behind the short end walls.
  for (const [wallY, dir] of [[yBase, -1], [yBaseT, 1]]) {
    e.push(poly(LAYERS.CUT, [
      [xBase + relief, wallY], [xBase - ear, wallY + dir * 2.5],
      [xBase - ear, wallY + dir * (wall - 2.5)],
  ...
  // Chamfered lid flaps — the fold-side corners stay square, only the outer
  // corners are cut back so the two flaps meet without fouling each other.
  const flapCut = (yFold, dir) => e.push(poly(LAYERS.CUT, [
    [xBase + relief, yFold],
    [xBase + relief, yFold + dir * (flap - chamfer)],
    [xBase + relief + chamfer, yFold + dir * flap],
    [xBaseR - relief - chamfer, yFold + dir * flap],
    [xBaseR - relief, yFold + dir * (flap - chamfer)],
    [xBaseR - relief, yFold]
  ]));
  flapCut(yFrontFold, -1);
  flapCut(yBackFold, 1);

  // Friction lock: a tab past the front flap's outer edge tucks through a
  // slot cut near the back flap's outer edge once the lid is folded shut.
  e.push(taperedFlap(LAYERS.CUT, centreX - tabW / 2, 0, tabW, tabDepth, tabTaper, false));
  e.push(rect(LAYERS.CUT, centreX - slotW / 2, top - slotInset - slotDepth, slotW, slotDepth));
  e.push(text(LAYERS.INFO, centreX, tabDepth + 3, 'LOCK TAB - tuck into slot', 2.6, 'middle'));
  e.push(text(LAYERS.INFO, centreX, top - slotInset - slotDepth - 4, 'LOCK SLOT', 2.6, 'middle'));

  e.push(text(LAYERS.DIM, centreX, yBase + W / 2, `${L} x ${W} x ${H} mm`, 5, 'middle'));
  e.push(text(LAYERS.DIM, centreX, yBase + W / 2 - 8,
    `board ${t} mm / allowance ${a} mm / self-locking, no tape`, 3.2, 'middle'));
  return e;
}

...
function legend(x, y) {
  const rows = [
    ...
    [LAYERS.SAFE, 'SAFE_ARTWORK — keep critical copy inside'],
...
function infoBlock(x, y, entry, params, thickness) {
  ...
  const lines = [
    ...
    'Scale — 1:1 in millimetres. Print at 100%; disable Fit to Page.',
    '',
    'PRODUCTION PREFLIGHT TEMPLATE — NOT RELEASED FOR TOOLING.',
    'Before tooling, the converter must approve stock caliper, grain direction,',
    'crease rule / matrix, glue and handle construction, machine tolerances,',
    'and a physical blank sample. Finished dimensions govern.',
  ...
  // A known-length calibration bar lets prepress catch accidental PDF scaling.
  const barY = y - lines.length * 5 - 2;
  e.push(line(LAYERS.DIM, x, barY, x + 100, barY));
  e.push(line(LAYERS.DIM, x, barY - 2, x, barY + 2));
  e.push(line(LAYERS.DIM, x + 100, barY - 2, x + 100, barY + 2));
  e.push(text(LAYERS.DIM, x + 50, barY + 3, '100 mm SCALE CHECK', 2.8, 'middle'));
  return e;
}
...
export function renderFormat(entry, params, format) {
  if (!['svg', 'dxf', 'pdf', 'ai'].includes(format)) {
    throw new Error(`Unsupported dieline format: ${format}`);
  }
  ...
  const pdf = toPDF(entities, {
    title,
    subject: `${entry.name} 1:1 mm production-preflight packaging template`,

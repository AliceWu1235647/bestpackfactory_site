...
import { tuckEndCarton, gableTopCarton, sleeve, pillowBox } from './templates-cartons.js';
import {
  mailerBox, trayBox, magneticRigidBox, paperBag, twoPieceRigidBox,
  wineBottleRigidBox, trayLidBox, selfLockingMailerBox
} from './templates-boxes.js';
import { standUpPouch, flatBottomPouch, flatPouch } from './templates-flexible.js';

export const GENERATORS = {
  tuckEndCarton, gableTopCarton, sleeve, pillowBox, mailerBox, trayBox,
  selfLockingMailerBox, magneticRigidBox, paperBag, twoPieceRigidBox, wineBottleRigidBox,
  trayLidBox, standUpPouch, flatBottomPouch, flatPouch
};
...
export const DIELINES = [
  {
    ...
    intro: 'Stand-up coffee pouch layout for a 250 g whole-bean fill, with adjustable degassing valve, resealable zipper and laser tear-notch guides for converter preflight.',
    fields: [
    ...
    seoDescription: 'Free 250g coffee bag engineering dieline with degassing valve, zipper and tear-notch guides. 10 preset sizes from 100g specialty to 2kg commercial. PDF, DXF, AI, SVG.',
  ...
  {
    slug: 'self-locking-mailer-box-dieline',
    name: 'Self-Locking Mailer Box Dieline (No Tape)',
    category: 'shipping',
    generator: 'selfLockingMailerBox',
    intro: 'Single-piece corrugated shipping box: the two long walls extend into a pair of chamfered lid flaps that meet at the centre and lock with a friction tab, so it closes shut without tape or glue.',
    fields: [
      F('length', 'Inner length', 250, 100, 600),
      F('width', 'Inner width', 180, 80, 450),
      F('height', 'Inner height', 80, 20, 300),
      F('thickness', 'Board thickness', 3, 2, 5, 0.5)
    ],
    presets: [
      { name: 'Small parcel', values: { length: 200, width: 150, height: 60 }, note: 'Accessories and small goods.' },
      { name: 'Standard mailer', values: { length: 250, width: 180, height: 80 }, note: 'General e-commerce default.' },
      { name: 'Book mailer', values: { length: 220, width: 160, height: 40 }, note: 'Hardcover and paperback books.' },
      { name: 'Apparel mailer', values: { length: 280, width: 220, height: 50 }, note: 'Folded garments, no hanger.' },
      { name: 'Subscription box', values: { length: 250, width: 250, height: 90 }, note: 'Multi-item monthly box.' },
      { name: 'Electronics mailer', values: { length: 300, width: 220, height: 100 }, note: 'Gadgets and accessory kits.' },
      { name: 'Shoe mailer', values: { length: 330, width: 210, height: 120 }, note: 'Sneakers and footwear shipping.' },
      { name: 'Large mailer', values: { length: 400, width: 300, height: 150 }, note: 'Multi-item or bulky shipments.' }
    ],
    materials: [
      'E-flute 1.5 mm — light parcels, best print surface',
      'B-flute 3 mm — the standard e-commerce compromise',
      'BE double wall 4.5 mm — heavy or fragile shipments'
    ],
    notes: [
      'Fold both end walls first, then the long walls, so the locking ears seat behind them.',
      'Fold the two lid flaps down last and push the tab through the slot to lock — no tape needed.',
      'Board allowance is applied automatically from the thickness you enter; re-check tab/slot clearance after changing thickness.'
    ],
    relatedProduct: '/products/custom-boxes.html',
    relatedLabel: 'Custom Shipping Boxes',
    seoDescription: 'Free self-locking mailer box dieline. No tape or glue — chamfered lid flaps interlock with a friction tab. 8 presets. PDF, DXF, AI, SVG.',
    keywords: ['self locking mailer box dieline', 'no tape shipping box template', 'wing lock mailer dieline', 'self locking carton dxf']
  },
  {
    ...
    category: 'bag',
    generator: 'paperBag',
    sizeDisplayKeys: ['length', 'width', 'height'],
    intro: 'Square-bottom retail bag engineering template with two side gussets, bottom-forming diagonal scores, a turn-over top, side glue seam, reinforcement guides and four handle punch positions.',
    fields: [
      F('length', 'Front width', 260, 80, 500),
      F('width', 'Side gusset', 120, 40, 250),
      F('height', 'Bag height', 320, 100, 600),
      F('hem', 'Top turn-over', 35, 20, 70),
      F('bottomFold', 'Bottom turn-in', 78, 30, 220, 1),
      F('seam', 'Side glue seam', 20, 12, 35, 0.5),
      F('paperThickness', 'Paper caliper', 0.2, 0.08, 0.5, 0.01),
      F('handleSpacing', 'Handle hole centres', 95, 35, 220, 1),
      F('handleHole', 'Handle hole diameter', 5, 3, 10, 0.5),
      BOOL('handles', 'Rope / ribbon handle holes', true)
    ],
    presets: [
      { name: 'Retail shopper', values: { length: 260, width: 120, height: 320, bottomFold: 78, handleSpacing: 95 }, note: 'Apparel and boutique retail.' },
      { name: 'Boutique small', values: { length: 180, width: 90, height: 230, bottomFold: 62, handleSpacing: 75 }, note: 'Cosmetics and accessories.' },
      { name: 'Wine bag', values: { length: 110, width: 110, height: 360, bottomFold: 73, handleSpacing: 60 }, note: 'Single bottle with rope handles.' },
      { name: 'Grocery bag', values: { length: 320, width: 170, height: 380, bottomFold: 106, handleSpacing: 110 }, note: 'Supermarket and grocery retail.' },
      { name: 'Gift bag small', values: { length: 160, width: 80, height: 200, bottomFold: 56, handleSpacing: 65 }, note: 'Small gifts and jewelry.' },
      { name: 'Gift bag medium', values: { length: 240, width: 100, height: 280, bottomFold: 67, handleSpacing: 90 }, note: 'Medium gift items.' },
      { name: 'Gift bag large', values: { length: 320, width: 130, height: 400, bottomFold: 84, handleSpacing: 110 }, note: 'Large gift boxes.' },
      { name: 'Bakery bag', values: { length: 200, width: 100, height: 240, bottomFold: 67, handleSpacing: 80 }, note: 'Bread and pastries.' },
      { name: 'Jewelry bag', values: { length: 140, width: 70, height: 180, bottomFold: 51, handleSpacing: 60 }, note: 'Jewelry boxes and small luxury.' },
      { name: 'Double wine bag', values: { length: 220, width: 110, height: 360, bottomFold: 73, handleSpacing: 85 }, note: 'Two-bottle wine carrier.' },
      { name: 'Shoe bag', values: { length: 300, width: 150, height: 360, bottomFold: 95, handleSpacing: 105 }, note: 'Shoe boxes and footwear.' }
    ],
    materials: [
    ...
    notes: [
      'The four diagonal bottom scores form two square-bottom diamonds; make a blank sample to confirm the selected turn-in on the converting line.',
      'The top turn-over doubles the paper at the handles. Confirm hole diameter, reinforcement patch and pull load for the chosen rope or ribbon.',
      'Artwork crossing a side-gusset centre score will fold out of alignment; keep logos inside the marked SAFE_ARTWORK panels.'
    ],
    relatedProduct: '/products/paper-bags.html',
    relatedLabel: 'Custom Paper Bags',
    seoDescription: 'Free square-bottom paper bag engineering dieline with gusset scores, bottom-forming diagonals, top turn-over, glue seam, reinforcement guides and handle holes. PDF, DXF, AI, SVG.',
  ...
  },
  {
    ...
    category: 'rigid',
    generator: 'twoPieceRigidBox',
    intro: 'Separate base and lid engineering layout for a classic rigid set-up box. One download includes both wrap sheets and the individual greyboard floor and wall pieces, with adjustable lid clearance, lid depth and paper turn-in.',
    seoDescription: 'Free two-piece rigid gift box engineering dieline with separate base and lid wrap sheets, greyboard pieces and adjustable lid clearance. PDF, DXF, AI, SVG. No sign-up.',
    fields: [
      F('length', 'Inner length', 200, 60, 600),
      F('width', 'Inner width', 150, 50, 500),
      F('height', 'Wall height', 50, 15, 200),
      F('thickness', 'Board thickness', 1.5, 0.3, 4, 0.1),
      F('lidClearance', 'Lid clearance', 3, 1, 8, 0.5),
      F('lidHeight', 'Lid wall height', 25, 12, 120, 1),
      F('turnIn', 'Wrap turn-in', 18, 10, 35, 1)
    ],
    presets: [
    ...
    notes: [
      'The download contains separate base and lid wrap blanks plus five greyboard pieces for each half; it is not a one-piece folding-carton net.',
      'Lid inside length and width include twice the selected greyboard caliper plus the selected clearance. Confirm the wrapped-board fit with a physical blank.',
      'Corner mitres, wrap paper grain, turn-in depth and adhesive depend on the converting line and require prepress approval.'
    ],
    relatedProduct: '/products/luxury-magnetic-boxes.html',
  ...
  },
  {
    ...
    intro: 'Tall rigid lift-off-lid box for single or double wine or spirit bottles. The download separates base and lid wrap sheets from greyboard pieces; double-bottle presets also include a glue-tab centre divider blank.',
    fields: [
      ...
      F('thickness', 'Board thickness', 1.5, 0.3, 4, 0.1),
      F('lidClearance', 'Lid clearance', 3, 1, 8, 0.5),
      F('lidHeight', 'Lid wall height', 70, 30, 180, 1),
      F('turnIn', 'Wrap turn-in', 18, 10, 35, 1),
      F('dividerTab', 'Divider glue tab', 15, 10, 25, 1),
      BOOL('divider', 'Include centre divider', false)
    ],
    presets: [
      ...
      { name: 'Double Bordeaux', values: { length: 230, width: 105, height: 350, divider: true }, note: 'Two standard bottles side by side.' },
      { name: 'Double Champagne', values: { length: 250, width: 115, height: 380, divider: true }, note: 'Two Champagne bottles.' },
    ...
    notes: [
      'Double-bottle presets include a divider at 70 % of the inside height; confirm its board grade and glue-tab width by transit testing.',
      'Bottle diameter, shoulder profile and closure height still require a fitted insert designed from the actual bottle sample.',
      'Confirm lid clearance on wrapped greyboard and complete a filled-pack drop test before releasing tooling.'
    ],
    relatedProduct: '/products/luxury-magnetic-boxes.html',
  ...
  },
  {
    ...
    name: 'Roll End Tray & Lid (Locking Corners)',
    category: 'shipping',
    generator: 'trayLidBox',
    seoDescription: 'Free two-piece roll-end tray and lid engineering dieline with locking corners and adjustable lid clearance. 10 presets for retail and transit packaging. PDF, DXF, AI, SVG.',
    intro: 'Two-piece corrugated tray and lid engineering layout with locking-corner ears. Both blanks are generated together, and lid clearance and wall depth can be adjusted for retail trays, board-game boxes and transit outers.',
    fields: [
      F('length', 'Inner length', 250, 80, 600),
      F('width', 'Inner width', 180, 60, 500),
      F('height', 'Tray height', 60, 20, 250),
      F('thickness', 'Board thickness', 3, 1, 7, 0.5),
      F('lidClearance', 'Lid clearance', 4.5, 2, 15, 0.5),
      F('lidHeight', 'Lid wall height', 22, 12, 100, 1)
    ],
    presets: [
    ...
    notes: [
      'The download generates separate locking-corner base and lid blanks; confirm ear length and relief gaps for the selected flute.',
      'Shelf-ready trays need a perforated tear-away front — mark it on the dieline before cutting.',
      'Lid clearance includes board caliper but must be checked on a folded blank because flute crush changes the finished fit.'
    ],
    relatedProduct: '/products/custom-boxes.html',
    relatedLabel: 'Custom Shipping Boxes',

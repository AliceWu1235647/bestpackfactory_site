...
import { resolveR2TypeLocation } from './r2-release';
...
async function productLocation() {
  return resolveR2TypeLocation('products', {
    prefix: env('R2_PRODUCT_JSON_PREFIX', 'products').replace(/^\/+|\/+$/g, ''),
    indexPath: env('R2_PRODUCT_INDEX_PATH', 'products/index.json').replace(/^\/+/, ''),
...
export async function getR2Product(slug) {
  ...
  const location = await productLocation();
  if (!location) return null;
  return fetchJson(
    `${baseUrl()}/${location.prefix}/${encodeURIComponent(clean)}.json`,
    [productTag(clean), ...(location.releaseId ? [`r2-release:${location.releaseId}`] : [])],
  );
}
...
export async function getR2ProductIndex() {
  ...
  if (!location) return null;
  return fetchJson(
    `${baseUrl()}/${location.indexPath}`,
    ['products:index', 'products-search', ...(location.releaseId ? [`r2-release:${location.releaseId}`] : [])],
  );
}

function siteUrl() {
  return env('NEXT_PUBLIC_SITE_URL', 'https://www.bestpackfactory.com')
...
export function metadataFromProduct(product, slug) {
  const clean = cleanProductSlug(slug);
  const title = product?.seoTitle || product?.title || product?.name || 'Custom Packaging Product | BestPackFactory';
  const description = product?.metaDescription || product?.description || product?.quickAnswer || 'Custom packaging product from BestPackFactory.';
  const canonical = product?.canonical || `${siteUrl()}/products/${clean}.html`;
...
<footer class="footer"><div><h3>BestPackFactory</h3><p>B2B custom packaging manufacturer for boxes, bags, labels, bottles, tins and printing.</p><p>Sales Manager:    Lisa Wu Email:    lisa@colorprintingpackage.com WhatsApp +86 158 8653 0985</p><p>Address: Huixin Zhichuang Park, 108 Huarong Road, Longhua District, Shenzhen, China</p></div><div><h3>Products</h3><a href="/products.html">All Products</a><a href="/products/luxury-magnetic-boxes.html">Magnetic Boxes</a><a href="/products/flexible-packaging.html">Flexible Packaging</a></div><div><h3>Inquiry</h3><a href="/contact.html">Request Quote</a><a href="mailto:lisa@colorprintingpackage.com">Email Lisa</a></div></footer>`;
}
